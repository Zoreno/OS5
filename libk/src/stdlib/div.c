 ...
 * Maintainer:
 * Created: Sat Feb 10 00:52:14 2018 (+0100)
 * Version:
 * Package-Requires: ()
 * Last-Updated: Sat Feb 10 00:52:38 2018 (+0100)
 *           By: Joakim Bertils
 *     Update #: 2
 * URL:
 * Doc URL:
...
static uint32_t divl(uint64_t num, uint32_t den)
{
...
{
    uint64_t num_abs = num >= 0 ? (uint64_t) num : -(uint64_t) num;
    uint64_t den_abs = den >= 0 ? (uint64_t) den : -(uint64_t) den;
